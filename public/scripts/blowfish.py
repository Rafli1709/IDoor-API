...
class Blowfish:
    ...
    def avalanche_effect(self, plaintext):
        ...
        for i in range(len(plaintext) * 8):
            ...
            modified_ciphertext = self.encrypt(modified_plaintext.decode('latin-1'))

            for j in range(len(original_ciphertext)):
                xor_result = int(original_ciphertext[j], 16) ^ int(modified_ciphertext[j], 16)
                avalanche_count += bin(xor_result).count('1')

        total_bits = len(plaintext) * 8 * len(original_ciphertext) * 4

        return [avalanche_count, total_bits]
